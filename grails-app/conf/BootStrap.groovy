...
class BootStrap {
    ...
    def init = { servletContext ->
		...
		def location = Location.first()
		location.events = [Event.first()]
		location.save()
		
		location = Location.find{ name == "Botequim Informal - Via Brasil" }
		location.events = [Event.last()]
		location.save()
    }
    def destroy = {
    }
